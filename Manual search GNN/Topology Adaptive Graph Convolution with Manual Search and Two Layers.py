...
output_dir = "./output/H8-16"  # Output directory for results and models
...
hyperparams = {
    ...
    'dropout': 0.6,
    'hidden_units1': 256,  # Number of hidden units in the first TAG layer
    'hidden_units2': 256,  # Number of hidden units in the second TAG layer
}
...
# Define two-layer TAG model
class TAGModel(nn.Module):
    def __init__(self, num_features, hidden_units1, hidden_units2, num_classes, dropout):
        super(TAGModel, self).__init__()
        self.tag_conv1 = TAGConv(num_features, hidden_units1)  # First TAGConv layer
        self.tag_conv2 = TAGConv(hidden_units1, hidden_units2)  # Second TAGConv layer
        self.classifier = nn.Linear(hidden_units2, num_classes)  # Classification layer
        self.dropout = dropout

    def forward(self, x, edge_index):
        x = self.tag_conv1(x, edge_index)  # First TAGConv layer
        x = F.relu(x)
        x = F.dropout(x, p=self.dropout, training=self.training)

        x = self.tag_conv2(x, edge_index)  # Second TAGConv layer
        x = F.relu(x)
        x = F.dropout(x, p=self.dropout, training=self.training)

        output = self.classifier(x)  # Classification layer
        return output

# Train TAG model and save node vectors
def train_TAG_model(data, labels, train_indices, val_indices, test_indices, hyperparams, output_dir):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # Compute class sample proportions to generate alpha
    class_counts = pd.Series(labels.cpu().numpy()).value_counts().sort_index()
    total_samples = len(labels)
    class_freq = class_counts / total_samples
    alpha = (1.0 / class_freq).values  # Avoid divide-by-zero and normalize alpha
...
if __name__ == '__main__':
    ...
    try:
        variables_file = os.path.join(input_dir, 'H8-16_variables.csv')  # Path to features file
        ...
        model, output_vectors, alpha = train_TAG_model(data, labels, train_indices, val_indices, test_indices, hyperparams, output_dir)
